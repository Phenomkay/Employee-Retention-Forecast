...
if st.button("Predict", key="predict_button"):
    # Convert inputs
    Work_accident = 1 if work_accident == "Yes" else 0  # Changed to 'Work_accident'
    ...
    input_data = pd.DataFrame({
        ...
        'Work_accident': [Work_accident],  # Changed to 'Work_accident'
    ...
    # No need to create a DataFrame here if the model wasn't trained with feature names
    # input_scaled_df = pd.DataFrame(input_scaled, columns=input_data.columns)

    # === Prediction ===
    prediction = model.predict(input_scaled)[0]  # Pass the NumPy array

    # === Prediction Probability ===
    try:
        class_index = list(model.classes_).index(1)
        prediction_proba = model.predict_proba(input_scaled)[0][class_index] # Pass the NumPy array
